...
#  define PRAGMA_LDTC_BEGIN(var, def, func, ...)                               \
    __dt_ldtc_begin(&var, def, &func, ##__VA_ARGS__)
...
#endif // #ifdef DT_ENABLE

template <typename V, typename F, typename... Args>
__attribute__((noinline, optnone)) void __dt_ldtc_begin(V *var,
                                                        V def,
                                                        F func,
                                                        Args... args) noexcept {
}

__attribute((noinline, optnone)) void __dt_ldtc_end() noexcept {}
